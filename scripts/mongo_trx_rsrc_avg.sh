...
mongoexport -d EOS -c trx_rsrc_avg_results_$limit_num -f _id.acct,_id.name,avg_cpu_us,avg_net_words,count --type=csv > $DIR/$file_name.csv
mongoexport -d EOS -c trx_rsrc_avg_results_$limit_num -f _id.acct,_id.name,avg_cpu_us,avg_net_words,count --type=json > $DIR/$file_name.json
